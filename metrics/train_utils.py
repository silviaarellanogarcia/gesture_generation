...
def create_video_and_save(save_path, epoch, prefix, iter_idx, target, output, mean_data, title,
    ...
    output_poses = data_utils.convert_dir_vec_to_pose(output)
    target_poses = None
    if target is not None:
        target = target + mean_data
        target_poses = data_utils.convert_dir_vec_to_pose(target)
    ...
    def animate(i):
        for k, name in enumerate(['human', 'generated']):
            ...
            if pose is not None:
                axes[k].clear()
                for j, pair in enumerate(data_utils.dir_vec_pairs):
